...
import CricketBall from "@/components/overlays/share/CricketBall";
...
// CricketBall's default, keyed off each celebration's accent color.
 ...
 *     included for demoing this in isolation — pass `hideDemoButtons` to
 *     suppress it once wired up to real ball-by-ball events.
 *
 ...
 */
export default function BoundaryCelebration({ hideDemoButtons = false }) {
  ...
  return (
    <>
      ...
      {mounted &&
        active &&
        createPortal(
          <div className={`bc-wrap fixed inset-0 z-[200] flex items-center justify-center overflow-hidden pointer-events-none ${closing ? "bc-closing" : ""}`}>
            <div
            ...
            {[0, 1, 2].map((i) => (
              <span
                ...
                style={{ width: 40, height: 40, border: `2px solid ${cfg.accent}`, animation: `bcRing 1s cubic-bezier(0.16,1,0.3,1) ${0.5 + i * 0.14}s both` }}
              />
            ))}

            {/* Flying cricket ball — shared sphere component, celebration's
                own warmer flight gradient instead of the default fill */}
            <CricketBall
              size={64}
      ...
      <style jsx global>{`
        ...
        @keyframes bcWipeOut { from { opacity: 1; transform: scale(1); } to { opacity: 0; transform: scale(1.06); } }
        @keyframes bcShake {
        ...
        @keyframes bcFlash { 0% { opacity: 0; } 25% { opacity: 1; } 100% { opacity: 0; } }
        @keyframes bcRing { 0% { opacity: 0.9; transform: scale(1); } 100% { opacity: 0; transform: scale(14); } }
        ...
        .bc-letter { animation: bcLetterIn 0.65s cubic-bezier(0.2, 1.6, 0.4, 1) both; }
        ...
        .bc-subtitle { opacity: 0; animation: bcSubtitleIn 0.5s cubic-bezier(0.22, 1, 0.36, 1) 1.05s both; }
        @keyframes bcSubtitleIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }

        .bc-particle { opacity: 0; animation: bcParticleOut 0.9s cubic-bezier(0.16, 1, 0.3, 1) both; }
        @keyframes bcParticleOut { 0% { opacity: 1; transform: translate(0, 0) scale(1); } 100% { opacity: 0; transform: translate(var(--tx), var(--ty)) scale(0.3); } }

        .bc-firework-spark { opacity: 0; animation: bcSparkOut 0.8s ease-out both; }
        @keyframes bcSparkOut { 0% { opacity: 1; transform: translate(0, 0) scale(1); } 100% { opacity: 0; transform: translate(var(--tx), var(--ty)) scale(0.2); } }

        .bc-confetti { opacity: 0; animation-name: bcConfettiFall; animation-timing-function: ease-in; animation-fill-mode: both; }
        ...
        @media (prefers-reduced-motion: reduce) {
          .bc-wrap, .bc-letter, .bc-subtitle, .bc-particle, .bc-firework-spark, .bc-confetti {
