...
// Reads a team logo off matchSetup. TeamInfo defines logoUrl (not
// "logo") — this reads that field directly instead of guessing at an
// alternate shape.
...
function PlayerCarousel({
  ...
  // Maps a player's name to whichever role they currently occupy
  // (striker / non-striker / bowler). Anyone with a role entry is
  // "already in play" and gets locked + badged in this carousel.
  ...
  // Everyone left in the squad is either dismissed or already occupying
  // a crease slot — nothing here is tappable. Show one calm summary
  // card instead of a wall of disabled chips. The actual "end innings"
  // action lives solely in the status card above the panel, so this
  // stays purely informational (no button) to avoid repeating it.
...
function CrewSlot({
  ...
  dismissedNames?: Set<string>;
  blockedName?: string;
  // True when the squad is confirmed exhausted and THIS slot is the
  // empty one with nobody left to send in. The action to resolve this
  // (end innings) lives solely in the status card above the panel —
  // this just explains *why* the slot is stuck, no button here.
...
export default function LiveStatePanel({
  ...
  // Resolves which team key ("teamA"/"teamB") the match result's winner
  // name corresponds to. Used for both the logo lookup and the team
  // color passed to the auto-fired Match Won moment.
  ...
  const winningTeamColor = winningTeamKey ? matchSetup?.[winningTeamKey]?.color : undefined;
  ...
  // Fires the Match Won moment automatically the instant the match
  // completes — no manual "confirm & fire" step. Guarded by a ref so
  ...
  return (
    <DrawerSection step="3" title="Scorer" description="Tap the ball, we do the maths" dirty={liveDirty} defaultOpen>
      ...
      {/* ── MATCH OVER: full takeover, everything else below is hidden ── */}
