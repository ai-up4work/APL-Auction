            ...
            {(["striker", "nonStriker"] as const).map((who) => (
              <div key={who} className="batter-card">
                ...
                <div className="flex items-center gap-2">
                  <span className="squad-avatar" style={{ width: 36, height: 36 }}>
                    {liveState[who].imageUrl ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={liveState[who].imageUrl} alt="" />
                    ) : (
                      <span className="squad-avatar-fallback">
                        {(liveState[who].name || (who === "striker" ? "ST" : "NS")).slice(0, 2).toUpperCase()}
                      </span>
                    )}
                  </span>
                  <input
