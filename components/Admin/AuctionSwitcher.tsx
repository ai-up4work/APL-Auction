...
export default function AuctionPicker({
  ...
  // FIX: creating an auction with no name used to silently fall back to
  // "APL Auction" for every user. Now a name is required — attempting to
  // submit blank shows an inline error and focuses the input instead of
  // proceeding into the dashboard.
  const [nameError, setNameError] = useState(false);

  // FIX: onCreateNew now actually writes to Supabase (see AuctionContext.
  // createNew) instead of only building local state, so it's async. This
  // tracks that in-flight state so the button shows "Creating…" and can't
  // be double-clicked into creating two auctions.
  ...
  async function attemptCreate() {
    ...
    try {
      ...
      // On success the parent flips `entered` and unmounts this picker,
      // so there's no need to reset isCreating here.
    } catch (err) {
      console.error("[AuctionPicker] failed to create auction:", err);
      setNameError(true);
      setIsCreating(false);
    }
  }
          ...
          <div className="flex-1 relative z-10">
            <input
              ...
              style={{
                ...
                border: `1px solid ${
                  nameError ? "var(--color-error)" : inputFocused ? "var(--color-theme-orange)" : "var(--color-border-overlay)"
                }`,
            ...
            {nameError && (
              <p
                ...
                style={{ color: "var(--color-error)", fontFamily: "var(--font-label-mono)" }}
              >
                {newName.trim()
                  ? "Couldn't create the auction — check your connection and try again."
                  : "Give your auction a name before creating it."}
              </p>
            )}
          </div>
          <button
            ...
            disabled={isCreating}
            className="relative z-10 px-5 py-2.5 rounded-xl text-sm font-black uppercase shrink-0 transition-all hover:-translate-y-0.5 disabled:opacity-70 disabled:hover:translate-y-0 disabled:cursor-wait flex items-center justify-center gap-2"
            style={{
