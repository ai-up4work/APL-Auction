// app/aution/[autionId]/screen/page.tsx
// ─────────────────────────────────────────────────────────────────────────────
// Broadcast / screen view — now a pure read-only display synced over Supabase
// realtime. The auctioneer page is the only thing that mutates auction_lots /
// bid_history; this page just listens and animates.
//
// NOTE: kept the existing folder param name "autionId" (typo) to avoid
// breaking routing — rename the folder to [auctionId] later if you want
// consistency with the /auctioneer/[auctionId] route.
// ─────────────────────────────────────────────────────────────────────────────
"use client";
...
import type { AuctionState, Player } from "@/types/auction";
...
// Shape used by FlowCanvas / ShuffleOverlay — borrowed from the mock dataset
// so we stay structurally compatible without knowing every field those
// components expect. Real values override id/name/img/price/status/team;
// anything else falls back to the template's placeholder value.
...
function ScreenContent({ auctionId }: { auctionId: string }) {
  ...
  const [shotClock, setShotClock] = useState(100); // 100 → 0, purely visual tension
  ...
  // Refs so realtime callbacks (registered once) always see fresh data
  const auctionRef = useRef<AuctionState | null>(null);
  const remainingRef = useRef<Player[]>([]);
  const currentLotRef = useRef<AuctionLot | null>(null);
  const completedLotsRef = useRef<AuctionLot[]>([]);
  ...
  useEffect(() => {
    async function init() {
      const [auctionData, liveData] = await Promise.all([
      ...
      for (const t of auctionData.teams) {
        if (t.supabaseId) {
          purses[t.supabaseId] = dbPurses[t.supabaseId] ?? {
            remaining: auctionData.rules.totalPoints,
            roster: t.roster,
          };
        }
      }
      ...
      setRemainingPlayers(auctionData.players.filter((p) => !usedIds.has(p.supabaseId ?? "")));

      if (liveData.currentLot?.status === "sold") setIsSold(true);
      if (liveData.currentLot?.status === "unsold") setIsUnsold(true);
      if (liveData.currentLot) setFlowActivePlayer(liveData.currentLot.playerId);
  ...
  // ── Purse bookkeeping when a lot closes sold (mirrors auctioneer's local update) ──
  const bumpPurseFor = useCallback((lot: AuctionLot) => {
    if (!lot.winningTeamId) return;
    setTeamPurses((prev) => {
      const fallback = { remaining: auctionRef.current?.rules.totalPoints ?? 0, roster: 0 };
      const old = prev[lot.winningTeamId!] ?? fallback;
      return {
        ...prev,
        [lot.winningTeamId!]: {
          remaining: old.remaining - lot.currentBid,
          roster: old.roster + 1,
        },
      };
    });
  ...
  // ── Shuffle animation → reveal the (already decided) lot ─────────────────
  const triggerShuffleAndReveal = useCallback((lot: AuctionLot) => {
    ...
    const pool: FlowPlayer[] = (candidates.length > 0 ? candidates : []).map((p) =>
      buildFlowPlayer({
    ...
    if (pool.length === 0) pool.push(targetFlow);
    setShufflePool(pool);

    let targetIndex = pool.findIndex((p) => p.id === targetFlow.id);
    ...
    function spin() {
      ...
      if (elapsed < SPIN_DURATION) {
        ...
        setTimeout(() => {
          ...
          setRemainingPlayers((prev) => prev.filter((p) => (p.supabaseId ?? "") !== lot.playerId));
          setFlowActivePlayer(lot.playerId);
          setShotClock(100);
        }, 900);
      }
    }
  ...
  }, [auctionId, triggerShuffleAndReveal, bumpPurseFor]);

  // ── Shot clock — visual tension only, never decides sold/unsold itself ───
  useEffect(() => {
    ...
    shotClockTimer.current = setInterval(() => {
      setShotClock((prev) => {
        ...
        const drainPerTick = 100 / (seconds * 10); // 10 ticks/sec @ 100ms interval
  ...
  // ── Occasional "Top Buys" leaderboard interrupt ──────────────────────────
  useEffect(() => {
  ...
  // ── Scroll the relevant Flow View row into focus when it changes ────────
  useEffect(() => {
    if (flowActivePlayer) {
      const el = document.getElementById(`player-${flowActivePlayer}`);
  ...
  useEffect(() => {
    if (flowActiveTeam) {
      const el = document.getElementById(`team-${flowActiveTeam}`);
      if (el) el.scrollIntoView({ behavior: "smooth", block: "center" });
    }
  }, [flowActiveTeam]);
  ...
  // Flow View data — best-effort mapping into the mock dataset's shape.
  // If FlowCanvas/AUCTION_CONFIG carry extra required fields beyond what's
  // overridden here, TypeScript will flag them — fill those in from your
  // real schema as needed.
    ...
    return auction.players.map((p) => {
      ...
      if (closedLot) {
        return buildFlowPlayer({
          ...
          img: p.img,
          price: `${p.price.toLocaleString()} CR`,
          status: closedLot.status === "sold" ? "sold" : "locked",
  ...
  if (!auction) {
    return (
      <div className="h-screen bg-[#0b0f10] flex items-center justify-center">
        <p className="font-mono-geist text-[#5a6a74] text-sm uppercase tracking-widest">Auction not found</p>
      </div>
    );
  }
  ...
  return (
    <>
      <style>{`
        ...
        .footer-blur { backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); }
        ...
        @media (max-width: 639px) {
          ...
          .team-card { flex: none !important; padding: 12px !important; width: 100% !important; }
      ...
      <div className="font-inter bg-[#0b0f10] text-[#e0e3e4] fixed inset-0 flex flex-col overflow-hidden select-none">
        ...
        <main className={`main-layout flex-1 mt-14 mb-[40px] flex overflow-hidden min-h-0 relative ${isSold && activeView === "live" ? "animate-shake" : ""}`}>
          ...
          {/* Passive sync indicator — control happens on the auctioneer page */}
          {activeView === "live" && (
            <div className="absolute bottom-6 right-6 z-[100] flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10">
              <span className="w-1.5 h-1.5 rounded-full bg-[#4ade80] animate-pulse" />
              <span className="font-mono-geist text-[9px] uppercase tracking-widest text-[#8a9ba8]">Synced with Auctioneer</span>
            </div>
          )}
...
export default function ScreenPage({
  ...
  params: Promise<{ autionId: string }>;
}) {
  const { autionId } = use(params);
  return <ScreenContent auctionId={autionId} />;
}