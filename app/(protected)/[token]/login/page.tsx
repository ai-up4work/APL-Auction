// app/(public)/invite/[token]/login/page.tsx
"use client"
...
export default function InviteLoginPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  // Same scroll-lock treatment as the main login page, for visual consistency.
  useEffect(() => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (signInError) {
      console.error("[invite-login] signIn failed:", {
    ...
    // Back to the accept-invite page — that's the only place acceptInvite
    // actually runs, so this is the one redirect target that matters here.
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <p className="mt-1.5 px-1 text-[11px] text-foreground/50">
                  Use the same email your invite was sent to.
                </p>
              </div>
            ...
            <p className="mt-2 text-center text-[11px] text-foreground/50">
              After creating your account, reopen your invite link to finish joining.
            </p>
          </div>
