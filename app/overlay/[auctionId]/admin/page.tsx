      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-8 py-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="panel p-5 lg:col-span-3 flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <button
              onClick={() => { const next = !powerplayOn; setPowerplayOn(next); fire({ type: "powerplay", show: next, label: "Powerplay &middot; Overs 1-6" }, `Powerplay badge ${next ? "on" : "off"}`); }}
              className={`fx-btn ${powerplayOn ? "fx-toggle-on" : "fx-toggle-off"}`}
            >
              Powerplay {powerplayOn ? "· on" : ""}
            </button>

            <button
              onClick={() => { const next = !pointsTableOn; setPointsTableOn(next); fire({ type: "pointsTable", show: next }, `Points table ${next ? "on" : "off"}`); }}
              className={`fx-btn ${pointsTableOn ? "fx-toggle-on" : "fx-toggle-off"}`}
            >
              Points table {pointsTableOn ? "· on" : ""}
            </button>

            <button
              onClick={() => {
                const next = !mainScoreboardOn;
                setMainScoreboardOn(next);
                fire({ type: "mainScoreboard", show: next, team: "MI", score: "187/4", overs: "18.3", crr: "10.10", target: "220", rrr: "6.00" }, `Main scoreboard ${next ? "on" : "off"}`);
              }}
              className={`fx-btn ${mainScoreboardOn ? "fx-toggle-on" : "fx-toggle-off"}`}
            >
              Main scoreboard {mainScoreboardOn ? "· on" : ""}
            </button>

            <button
              onClick={() => {
                const next = !battingOn;
                setBattingOn(next);
                fire({ type: "batting", show: next, batsmen: [{ name: "Rohit Sharma", runs: 78, balls: 46, onStrike: true }, { name: "Ishan Kishan", runs: 34, balls: 28 }] }, `Batting panel ${next ? "on" : "off"}`);
              }}
              className={`fx-btn ${battingOn ? "fx-toggle-on" : "fx-toggle-off"}`}
            >
              Batting panel {battingOn ? "· on" : ""}
            </button>

            <button
              onClick={() => { const next = !bowlingOn; setBowlingOn(next); fire({ type: "bowling", show: next }, `Bowling panel ${next ? "on" : "off"}`); }}
              className={`fx-btn ${bowlingOn ? "fx-toggle-on" : "fx-toggle-off"}`}
            >
              Bowling panel {bowlingOn ? "· on" : ""}
            </button>

            <button
              onClick={() => { const next = !partnershipOn; setPartnershipOn(next); fire({ type: "partnership", show: next, runs: 62, balls: 41 }, `Partnership tracker ${next ? "on" : "off"}`); }}
              className={`fx-btn ${partnershipOn ? "fx-toggle-on" : "fx-toggle-off"}`}
            >
              Partnership {partnershipOn ? "· on" : ""}
            </button>

            <button
              onClick={() => { const next = !overSummaryOn; setOverSummaryOn(next); fire({ type: "overSummary", show: next, balls: ["1", "4", "W", ".", "6", "2"] }, `Over summary ${next ? "on" : "off"}`); }}
              className={`fx-btn ${overSummaryOn ? "fx-toggle-on" : "fx-toggle-off"}`}
            >
              Over summary {overSummaryOn ? "· on" : ""}
            </button>

            <button
              onClick={() => { const next = !playerOfMatchOn; setPlayerOfMatchOn(next); fire({ type: "playerOfMatch", show: next }, `Player of the match ${next ? "on" : "off"}`); }}
              className={`fx-btn ${playerOfMatchOn ? "fx-toggle-on" : "fx-toggle-off"}`}
            >
              Player of match {playerOfMatchOn ? "· on" : ""}
            </button>
          ...
          <div className="grid grid-cols-2 gap-3 mt-1">
            <button
              onClick={() => fire({ type: "milestone", player: "V Kohli", milestone: "50", ballsTaken: 34 }, "Milestone: 50")}
              className="fx-btn"
              style={{ background: "rgba(232,196,104,0.12)", color: "#E8C468" }}
            >
              Fire milestone popup
            </button>
            <button
              onClick={() => fire({ type: "wicket", batsman: "R Sharma", dismissal: "c Rahul b Bumrah", runs: 78, balls: 46 }, "Wicket: R Sharma")}
              className="fx-btn"
