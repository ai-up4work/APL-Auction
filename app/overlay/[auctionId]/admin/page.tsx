...
export default function OverlayAdminPage({ params }: { params: Promise<{ auctionId: string }> }) {
  ...
  const overlayUrl =
    typeof window !== "undefined" ? `${window.location.origin}/overlay/${auctionId}` : "";
    ...
    >
      ...
      <header className="flex items-center justify-between px-8 h-16 border-b border-white/10 shrink-0">
        <div className="flex items-center gap-3">
          <span className="material-symbols-outlined text-theme-orange" style={{ fontSize: 22 }}>
            stream
          </span>
          <h1 className="font-archivo text-xl font-bold italic uppercase tracking-tight text-theme-orange">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-8 py-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="panel p-5 lg:col-span-3 flex items-center justify-between gap-4 flex-wrap">
          <div>
            <div className="font-mono-geist text-[9px] uppercase tracking-[0.2em] text-white/50 mb-1">
          ...
          <div className="flex items-center gap-3">
            <span className="font-mono-geist text-[9px] text-white/40 uppercase tracking-widest">
              1920×1080 · transparent bg on
            </span>
            <button
              onClick={copyUrl}
              className="fx-btn"
              style={{ background: "linear-gradient(135deg,#A87815,#E8C468)", color: "#1a1304", border: "none" }}
            >
        ...
        <div className="panel p-5 flex flex-col gap-3">
          <h3 className="font-mono-geist text-[10px] uppercase tracking-[0.2em] text-white/60 font-bold mb-1">
            Quick FX
          </h3>
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={() => fire({ type: "stamp", state: "sold" }, "Stamp: SOLD")}
              className="fx-btn"
            ...
            <button
              onClick={() => fire({ type: "stamp", state: "unsold" }, "Stamp: UNSOLD")}
              className="fx-btn"
              style={{ background: "rgba(113,128,150,0.14)", color: "#CBD5E0", borderColor: "rgba(113,128,150,0.3)" }}
            >
              Unsold Stamp
            </button>
            <button
              onClick={() => fire({ type: "confetti" }, "Confetti burst")}
              className="fx-btn"
              style={{ background: "rgba(232,196,104,0.12)", color: "#E8C468" }}
            >
              Confetti
            </button>
            <button
              onClick={() => fire({ type: "flash", color: "rgba(201,151,31,0.18)" }, "Flash")}
              className="fx-btn"
            ...
            <button
              onClick={() => fire({ type: "replayBadge", show: true }, "Replay badge ON")}
              className="fx-btn"
              style={{ background: "rgba(239,68,68,0.14)", color: "#f87171", borderColor: "rgba(239,68,68,0.3)" }}
            >
              Replay On
            </button>
            <button
              onClick={() => fire({ type: "replayBadge", show: false }, "Replay badge OFF")}
              className="fx-btn"
        ...
        <div className="panel p-5 flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-3 mt-1">
            <button
              onClick={() => {
                setLtShowing(true);
                fire({ type: "lowerThird", show: true, tag: ltTag, title: ltTitle, subtitle: ltSubtitle }, `Lower third: ${ltTitle}`);
              }}
              className="fx-btn"
              style={{ background: "linear-gradient(135deg,#A87815,#E8C468)", color: "#1a1304", border: "none" }}
            >
            ...
            <button
              onClick={() => {
                setLtShowing(false);
                fire({ type: "lowerThird", show: false }, "Lower third hidden");
              }}
              className="fx-btn"
              style={{ background: "rgba(255,255,255,0.05)", color: "#a0aec0" }}
            >
          ...
          <span className="font-mono-geist text-[9px] uppercase tracking-widest" style={{ color: ltShowing ? "#22c55e" : "#666" }}>
            {ltShowing ? "● live on overlay" : "○ hidden"}
          </span>
        </div>

        {/* ── Scoreboard (mock score / bid) ──────────────────────────────── */}
        <div className="panel p-5 flex flex-col gap-3">
          <h3 className="font-mono-geist text-[10px] uppercase tracking-[0.2em] text-white/60 font-bold mb-1">
            Scoreboard / Bid Update
          </h3>
          ...
          <div className="grid grid-cols-2 gap-3 mt-1">
            <button
              onClick={() => {
                setSbShowing(true);
                fire({ type: "scoreboard", show: true, label: sbLabel, value: sbValue, sub: sbSub }, `Scoreboard: ${sbValue}`);
              }}
              className="fx-btn"
              style={{ background: "linear-gradient(135deg,#A87815,#E8C468)", color: "#1a1304", border: "none" }}
            >
            ...
            <button
              onClick={() => {
                setSbShowing(false);
                fire({ type: "scoreboard", show: false }, "Scoreboard hidden");
              }}
              className="fx-btn"
              style={{ background: "rgba(255,255,255,0.05)", color: "#a0aec0" }}
            >
          ...
          <span className="font-mono-geist text-[9px] uppercase tracking-widest" style={{ color: sbShowing ? "#22c55e" : "#666" }}>
            {sbShowing ? "● live on overlay" : "○ hidden"}
          </span>
        </div>

        {/* ── Sold Board (running scorecard) ─────────────────────────────── */}
        <div className="panel p-5 flex flex-col gap-3">
          <h3 className="font-mono-geist text-[10px] uppercase tracking-[0.2em] text-white/60 font-bold mb-1">
            Sold Board (Scorecard)
          </h3>
            ...
            <button
              onClick={async () => {
                ...
                const { data, error } = await supabase
                  .from("overlay_sold_board")
                  .insert({ auction_id: auctionId, team: sbTeam, player: sbPlayer, price: sbPrice })
                  .select("id")
                  .single();
                fire(
                  {
                    type: "soldBoard",
                    show: true,
                    entry: { id: error ? `${Date.now()}` : data.id, team: sbTeam, player: sbPlayer, price: sbPrice },
                  },
                  `Sold board: ${sbPlayer} → ${sbTeam}${error ? " (not saved — check table)" : ""}`
                );
              }}
              className="fx-btn"
              style={{ background: "linear-gradient(135deg,#A87815,#E8C468)", color: "#1a1304", border: "none" }}
          ...
          <span className="font-mono-geist text-[9px] uppercase tracking-widest" style={{ color: soldBoardShowing ? "#22c55e" : "#666" }}>
            {soldBoardShowing ? "● live on overlay" : "○ hidden"}
          </span>
        </div>
        ...
        <div className="panel p-5 flex flex-col gap-3 lg:col-span-3">
          ...
          <div className="flex gap-3">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && tickerMsg.trim()) {
                  fire({ type: "ticker", message: tickerMsg.trim() }, `Ticker: ${tickerMsg.trim()}`);
                }
              }}
            />
            <button
              onClick={() => tickerMsg.trim() && fire({ type: "ticker", message: tickerMsg.trim() }, `Ticker: ${tickerMsg.trim()}`)}
              className="fx-btn shrink-0 px-6"
              style={{ background: "linear-gradient(135deg,#A87815,#E8C468)", color: "#1a1304", border: "none" }}
            >
              Send
            </button>
        ...
        <div className="panel p-5 lg:col-span-3">
          <h3 className="font-mono-geist text-[10px] uppercase tracking-[0.2em] text-white/60 font-bold mb-3">
            Recent triggers
          </h3>
          {log.length === 0 ? (
            ...
            <div className="space-y-1.5 max-h-40 overflow-y-auto">
              {log.map((l, i) => (
                <div key={i} className="font-mono-geist text-[11px] text-white/50">{l}</div>
              ))}
            </div>
          )}
        </div>
